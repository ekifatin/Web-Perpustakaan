...
class PinjamController extends Controller
{
    ...
    public function index()
    {
        ...
        $user = User::all();
        $dns = Auth::user()->name;
        $pinjam = Pinjam::join('buku', 'pinjam.id_buku', '=', 'buku.id')
                ->select('buku.nama_buku', 'pinjam.id_buku', 'pinjam.nama_pinjam', 'pinjam.nomor_hp', 'pinjam.status', 'pinjam.tanggal_pinjam', 'pinjam.tanggal_kembali', 'buku.gambar', 'pinjam.id')
    ...
    public function destroy($id)
    {
        ...
        $file = public_path('fotobuku/').$pinjam->gambar;
        if(file_exists($file)){
            @unlink($file);
        }
        $pinjam->delete();
